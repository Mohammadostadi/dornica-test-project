...
if (isset($_GET['member'])) {
    $member_order = 'orders.member_id = ' . securityCheck($_GET['member']);
}
    ...
    <div class="wrapper">
        ...
        <main class="page-content">
            ...
            <div class="page-breadcrumb   d-sm-flex align-items-center mb-3">
                ...
                <div class="ms-auto">
                    <div class="btn-group">
                        <?php if (isset($member_order) and !empty($member_order)) { ?>
                                        ...
                                        <table class="table">
                                            <thead class="text-center">
                                                <tr>
                                                    ...
                                                    <th class="px-5">
                                                        <a href="<?= sort_link('shipping') ?>"
                                                            class="sort-table <?= sortActive('shipping') ?>"></a>
                                                        روش ارسال
                                                    </th>
                                                    <th class="px-5">
                                            ...
                                            <tbody class="text-center">
                                                ...
                                                <tr id="<?= (isset($_SESSION['order_filter']['order']) and !empty($_SESSION['order_filter']['order'])) ? "" : "filter-row" ?>"
                                                    class="<?= (isset($_SESSION['order_filter']['order']) and !empty($_SESSION['order_filter']['order'])) ? "" : "d-none" ?>">
                                                    <form class=" d-flex justify-content-around align-content-start"
                                                    id="form" action="orders_list.php?page=1" method="post">
                                                        <td></td>
                                                        <td class="col-lg-2 col-md-4"> <select
                                                                class="form-select text-secondary"
                                                                name="orders_member_id" id="orders_member_id">
                                                                <option value="" class="text-secondary">لیست کاربران
                                                                </option>
                                                                <?php foreach ($membersList as $member) { ?>
                                                                    <option
                                                                        <?= (isset($_SESSION['order_filter']['orders_member_id']) and $_SESSION['order_filter']['orders_member_id'] == $member['id']) ? "SELECTED" : "" ?> value="<?= $member['id'] ?>">
                                                        ...
                                                        <td class="col-lg-2 col-md-4"> <input class="col form-control"
                                                                type="text"
                                                                value="<?= isset($_SESSION['order_filter']['products_name']) ? $_SESSION['order_filter']['products_name'] : "" ?>"
                                                                name="products_name" placeholder="نام محصول"> </td>
                                                                <td></td>
                                                        <td class="col-lg-2 col-md-4"> <input id="date"
                                                                class="col form-control" type="text"
                                                                value="<?= isset($_SESSION['order_filter']['orders_setdate']) ? $_SESSION['order_filter']['orders_setdate'] : "" ?>"
                                                                name="orders_setdate" placeholder="تاریخ"> </td>
                                                        <td class="col-lg-2 col-md-4"> <select
                                                                class="form-select text-secondary"
                                                                name="orders_paymentTypeId" id="orders_paymentTypeId">
                                                                <option value="" class="text-secondary">روش پرداخت
                                                                </option>
                                                                <?php foreach ($paymentList as $payment) { ?>
                                                                    <option
                                                                        <?= (isset($_SESSION['order_filter']['orders_paymentTypeId']) and $_SESSION['order_filter']['orders_paymentTypeId'] == $payment['id']) ? "SELECTED" : "" ?> value="<?= $payment['id'] ?>">
                                                                        <?= $payment['name'] ?>
                                                                    </option>
                                                                <?php } ?>
                                                            </select></td>
                                                        <td class="col-lg-2 col-md-4"> <select
                                                                class="form-select text-secondary"
                                                                name="orders_shippingTypeId" id="orders_shippingTypeId">
                                                                <option value="" class="text-secondary">روش ارسال
                                                                </option>
                                                                <?php foreach ($shippingList as $shipping) { ?>
                                                                    <option
                                                                        <?= (isset($_SESSION['order_filter']['orders_shippingTypeId']) and $_SESSION['order_filter']['orders_shippingTypeId'] == $shipping['id']) ? "SELECTED" : "" ?> value="<?= $shipping['id'] ?>">
                                                                        <?= $shipping['name'] ?>
                                                                    </option>
                                                                <?php } ?>
                                                            </select></td>
                                                        ...
                                                        <td class="col-lg-2 col-md-4"> <input class="col form-control"
                                                                type="text"
                                                                value="<?= isset($_SESSION['order_filter']['orders_ordersCode']) ? $_SESSION['order_filter']['orders_ordersCode'] : "" ?>"
                                                                name="orders_ordersCode" placeholder="کد سفارش"> </td>
                                                        <td class="col-lg-2 col-md-4"> <select
                                                                class="form-select text-secondary" name="orders_status"
                                                                id="status">
                                                                <option value="" class="text-secondary">وضعیت</option>
                                                                <option
                                                                    <?= (isset($_SESSION['order_filter']['orders_status']) and $_SESSION['order_filter']['orders_status'] == 0) ? 'selected' : '' ?> value="0">درحال بررسی</option>
                                                                <option
                                                                    <?= (isset($_SESSION['order_filter']['orders_status']) and $_SESSION['order_filter']['orders_status'] == 1) ? 'selected' : '' ?> value="1">خوانده شده</option>
                                                                <option
                                                                    <?= (isset($_SESSION['order_filter']['orders_status']) and $_SESSION['order_filter']['orders_status'] == 2) ? 'selected' : '' ?> value="2">خوانده نشده</option>
                                                            </select> </td>
                                                        <td class="col-12 text-end button-filter">
