...
<div class="wrapper">
    ...
    <?php
        ...
        <div class="page-breadcrumb   d-sm-flex align-items-center mb-3">
            ...
            <div class="ms-auto">
                <div class="btn-group">
                    <a class="btn btn-outline-secondary" href="payment_type_add.php"> اضافه کردن داده جدید</a>
                    ...
                    <div class="col-12 d-flex">
                        <div class="card border shadow-none w-100">
                            ...
                            <div class="card-header">
                                <div id="<?= (isset($_SESSION['shipping_type_filter']['shipping_type']) and !empty($_SESSION['payment_type_filter']['payment_type']))?"":"filter-row"?>" class="<?= (isset($_SESSION['shipping_type_filter']['shipping_type']) and !empty($_SESSION['payment_type_filter']['payment_type']))?"":"d-none"?>">
                                    <form class="" id="form" action="payments_type.php?page=1" method="post" >
                                        <div class="row g-3">
                                    <div class="col-lg-2 col-md-4" > <input class="col form-control" type="text" value="<?= isset($_SESSION['payment_type_filter']['name'])?$_SESSION['payment_type_filter']['name']:"" ?>" name="name" placeholder="عنوان" > </div>
                                    <div class="col-lg-2 col-md-4" > <select class="form-select text-secondary" name="status" id="status">
                                        ...
                                        <option <?= (isset($_SESSION['payment_type_filter']['status']) and $_SESSION['payment_type_filter']['status']== 1) ? 'selected' : '' ?> value="1">فعال</option>
                                        <option <?= (isset($_SESSION['payment_type_filter']['status']) and $_SESSION['payment_type_filter']['status'] == 0) ? 'selected' : '' ?> value="0">غیر فعال</option>
                                    </select> </div>
                                    ...
                                    <thead class="table-light text-center">
                                        ...
                                        <?php foreach($res as $payment) { ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <div>
                                                        <a href="payment_type_update.php?id=<?= $payment['id'] ?>" class="text-warning" data-bs-toggle="tooltip" data-bs-placement="bottom" title="ویرایش اطلاعات" data-bs-original-title="ویرایش اطلاعات" aria-label="Edit"><i class="bi bi-pencil-fill"></i></a>
                                                        <?php
                                                            $res = $db->where('paymentTypeId', $payment['id'])
                                                            ->getValue('orders', 'COUNT(*)');
                                                        ?>
                                                        <button class="<?= !empty($res)?"disabled text-secondary":"open-confirm text-danger" ?> btn border-0" value="<?= $payment['id'] ?>" data-bs-toggle="tooltip" data-bs-placement="bottom" title="<?= !empty($res)?"قابل حذف نیست":"حذف" ?>" data-bs-original-title="حذف" aria-label="Delete"><i class="bi bi-trash-fill"></i></button>
                                                    </div>
                                                </td>
                                            </tr>
