                    ...
                    <div class="col-12 d-flex">
                        <div class="card border shadow-none w-100">
                            ...
                            <div class="card-header">
                                <div id="<?= (isset($_SESSION['province_filter']['province']) and !empty($_SESSION['province_filter']['province']))?"":"filter-row"?>" class="<?= (isset($_SESSION['province_filter']['province']) and !empty($_SESSION['province_filter']['province']))?"":"d-none"?>">
                                    <form class="" id="form" action="provinces_list.php?page=1" method="post" >
                                        <div class="row g-2">
                                    <div class="col-lg-2 col-md-4" > <input class="col form-control" type="text" value="<?= isset($_SESSION['province_filter']['name'])?$_SESSION['province_filter']['name']:"" ?>" name="name" placeholder="عنوان" > </div>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="d-flex align-items-center gap-3 fs-6">
                                                        <a href="province_update.php?id=<?= $province['id'] ?>" class="text-warning" data-bs-toggle="tooltip" data-bs-placement="bottom" title="ویرایش اطلاعات" data-bs-original-title="ویرایش اطلاعات" aria-label="Edit"><i class="bi bi-pencil-fill"></i></a>
                                                        <?php
                                                            $res = $db->where('province_id', $province['id'])
                                                            ->getValue('cities', 'COUNT(*)');
                                                        ?>
                                                        <button class="<?= !empty($res)?"disabled text-secondary":'open-confirm text-danger' ?> btn border-0" 
                                                        value="<?= $province['id'] ?>" data-bs-toggle="tooltip" data-bs-placement="bottom" title="<?= !empty($res)?"قابل حذف نیست":"حذف" ?>" data-bs-original-title="حذف" aria-label="Delete">
